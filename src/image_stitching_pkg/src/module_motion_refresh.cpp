...
// 添加了模块功能: 运动物体检测和对应的拼接模板刷新
// 添加computeNeighborROI,checkNeighborROI对重叠区域进行运动物体检测，来更新拼接模板的功能，目的是为了在不牺牲太多拼接速度的同时保证视频质量。所以检测方法并不需要太复杂，只需实用即可。
#include <iostream>
#include <fstream>
#include <string>
...
Rect computePanoROI(const vector<Point> &corners, const vector<Size> &sizes)
{
...
struct TransformationData {
    ...
    Ptr<Blender> blender;
    Rect roi;
    vector<cv::Rect> overlaps;
};


// 缓存每个overlap区域的前一帧数据
std::vector<Mat> prev_hist_rdata_list;  // 与overlaps一一对应


// 计算第i个和第i+1个矩形的重叠区域
cv::Rect getOverlapRect(
...
{
    // 确保在有效范围内
    int dx_A = corners[i].x - dst_roi.x;// 左上角
    int dy_A = corners[i].y - dst_roi.y;
    int dx_B = corners[i + 1].x - dst_roi.x; // 下一个矩形的左上角
    int dy_B = corners[i + 1].y - dst_roi.y;

    // 获取矩形A（i）的参数
    
    int a_x1 = dx_A;
    int a_y1 = dy_A;
    int a_x2 = a_x1 + images_sizes[i].width - 1; // 右下角x
    int a_y2 = a_y1 + images_sizes[i].height - 1; // 右下角y

    // 获取矩形B（i+1）的参数
    int b_x1 = dx_B;
    int b_y1 = dy_B;
    int b_x2 = b_x1 + images_sizes[i+1].width - 1; // 右下角x
    int b_y2 = b_y1 + images_sizes[i+1].height - 1; // 右下角y

    // 计算重叠区域的边界
    int overlap_x1 = std::max(a_x1, b_x1);
    int overlap_y1 = std::max(a_y1, b_y1);
    int overlap_x2 = std::min(a_x2, b_x2);
    int overlap_y2 = std::min(a_y2, b_y2);

    // 构造重叠区域的Rect（左上角、宽、高）
    return cv::Rect(
        overlap_x1,
        overlap_y1,
        overlap_x2 - overlap_x1 + 1, // 宽
        overlap_y2 - overlap_y1 + 1  // 高
    );
}

// 批量计算所有相邻矩形的重叠区域
vector<cv::Rect> getAllAdjacentOverlaps(const std::vector<cv::Point>& corners,const std::vector<cv::Size>& images_sizes) 
{
    Rect dst_roi =computePanoROI(corners, images_sizes); // 计算全景图的ROI

    std::vector<cv::Rect> overlaps;
    if (corners.size() < 2 || images_sizes.size() != corners.size()) {
        return overlaps; // 输入无效（不足2个矩形）
    }
    for (int i = 0; i < corners.size() - 1; ++i) {
        overlaps.push_back(getOverlapRect(corners, images_sizes, i, dst_roi));
    }
    cout << "计算重叠区域完成，共有 " << overlaps.size() << " 个重叠区域。" << endl;
    // 输出每个重叠区域的坐标和大小
    for (size_t i = 0; i < overlaps.size(); ++i) {
        cout << "重叠区域 " << i + 1 << ": "
             << "左上角(" << overlaps[i].x << ", " << overlaps[i].  y << "), "
             << "宽=" << overlaps[i].width << ", "
             << "高=" << overlaps[i].height << endl;
    }
    // 返回所有重叠区域
    return overlaps;
}
...
// 初始化第一帧的重叠区域数据
void initFirstFrame(const cv::Mat& first_frame, const std::vector<cv::Rect>& overlaps) {
    prev_hist_rdata_list.clear();
    for (const auto& rect : overlaps) {
        Mat prev_gray;
        // 1. 提取overlap区域并转灰度
        cv::Mat current_roi = first_frame(rect);  // 提取区域（可能是彩色）
        cvtColor(current_roi, prev_gray, cv::COLOR_BGR2GRAY);  // 转为灰度
        
        // 2. 计算并缓存灰度直方图
        Mat prev_hist = computeGrayHistogram(prev_gray);
        
        prev_hist_rdata_list.push_back(prev_hist);
    }
}

// 辅助函数：计算灰度图的直方图（ bins=256，范围[0,256)）
cv::Mat computeGrayHistogram(const cv::Mat& gray_img) {
    int histSize = 256;
    float range[] = {0, 256};
    const float* histRange = {range};
    cv::Mat hist;
    calcHist(&gray_img, 1, 0, cv::Mat(), hist, 1, &histSize, &histRange, true, false);
    normalize(hist, hist, 0, 1, cv::NORM_MINMAX, -1, cv::Mat());  // 归一化（便于对比）
    return hist;
}

/**
 * 直方图统计法检测运动物体（相关法计算相似度）
 * @param current_gray 当前帧overlap区域的灰度图
 * @param prev_hist 前一帧该区域的灰度直方图
 * @param sim_threshold 相似度阈值η（如0.7，小于此值则判定有运动）
 * @return 是否检测到运动物体（true=有运动）
 */
bool detectByHistStat(const cv::Mat& current_gray, const cv::Mat& prev_hist, 
                     double sim_threshold) {
    // 1. 计算当前区域的灰度直方图
    cv::Mat current_hist = computeGrayHistogram(current_gray);
    
    // 2. 用相关法计算与前一帧直方图的相似度（范围[-1,1]）
    double similarity = cv::compareHist(prev_hist, current_hist, cv::HISTCMP_CORREL);
    
    // 3. 相似度小于阈值则判定有运动（直方图差异大）
    return (similarity < sim_threshold);
}

/**
 * 单次刷新时处理所有overlap区域，返回每个区域的检测结果
 * @param current_frame 当前帧图像（彩色，如BGR格式）
 * @param overlaps 所有重叠区域的Rect参数（固定）
 * @param prev_hist_rdata_list 前一帧的缓存数据（需提前初始化）
 * @param diff_threshold 帧差法的差值阈值η
 * @param pixel_threshold 帧差法的像素数量阈值
 * @param sim_threshold 直方图法的相似度阈值η
 * @return 每个overlap区域的检测结果（是否要重新检测的布尔值）
 */
bool processFrame(const cv::Mat& current_frame, 
                                      const std::vector<cv::Rect>& overlaps,
                                      std::vector<Mat>& prev_hist_rdata_list,
                                      double sim_threshold) {
    bool has_motion = false; // 初始化为无需要检测的状态
    
    // 遍历每个重叠区域
    for (size_t i = 0; i < overlaps.size(); ++i) {
        cv::Rect overlap_rect = overlaps[i];
        
        // 1. 提取当前帧中overlap区域的图像
        cv::Mat current_roi = current_frame(overlap_rect);
        
        // 2. 转为灰度图
        cv::Mat current_gray;
        cvtColor(current_roi, current_gray, cv::COLOR_BGR2GRAY);
        
        // 3. 直方图统计法检测（需前一帧的直方图）
        bool current_motion = detectByHistStat(
            current_gray, 
            prev_hist_rdata_list[i], 
            sim_threshold
        );
        
        if (current_motion) has_motion = true; // 只要有一个区域有运动，整体标记为有运动

        // 4. 缓存当前帧数据，供下一帧使用
        prev_hist_rdata_list[i] = computeGrayHistogram(current_gray);
...
// 保存和剪裁第一组图像融合结果
    ...
    vector<cv::Rect> overlaps = getAllAdjacentOverlaps(corners, sizes); // 计算所有相邻矩形的重叠区域
// ——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
    ...
    data.roi = roi; 
    data.overlaps = overlaps; // 保存重叠区域信息
    return data;
}
