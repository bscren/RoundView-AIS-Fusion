...
def multi_proc_worker(input_queue, output_queue, im_shape, t, max_dis, skip_interval = 1, camera_type = "normal", yolo_type = "yolo11m"):
    ...
    while True:
        ...
        try:
            ...
            try:
                output_queue.put(result, timeout=0.1)  # 添加超时防止阻塞
            except:
            ...
            print(error_msg)
            try:
                output_queue.put({"cam_idx": cam_idx, "error": error_msg}, timeout=0.1)
            except:
                pass
# ------------------------------------------------------
...
class AisVisNode(Node):
    def __init__(self,):
        ...
        self.mp_output_queues = [Queue(maxsize=20) for _ in range(self.num_cameras)]  # 增大输出队列
        self.workers = [
            Process(
        ...
        # 添加监控计时器
        self.monitor_timer = self.create_timer(5.0, self.monitor_performance)  # 每5秒监控一次性能
        self.frame_counts = {}  # 记录每个相机的处理帧数
        self.drop_counts = {}   # 记录每个相机的丢弃帧数
        for i in range(self.num_cameras):
            self.frame_counts[i] = 0
            self.drop_counts[i] = 0
    ...
    def refresh_window_callback(self):
        ...
        for cam_idx in range(self.num_cameras):
            try:
                processed_results = 0
                while processed_results < 10:  # 限制每次最多处理10个结果，防止长时间占用
    ...
    def monitor_performance(self):
        ...
        total_drops = sum(self.drop_counts.values())
        
        if total_frames > 0:
            drop_rate = total_drops / total_frames * 100
            self.get_logger().info(f"=== 性能监控 ===")
            self.get_logger().info(f"总处理帧数: {total_frames}, 总丢弃帧数: {total_drops}, 丢弃率: {drop_rate:.2f}%")
            
            for i in range(self.num_cameras):
                if self.frame_counts[i] > 0:
                    rate = self.drop_counts[i] / self.frame_counts[i] * 100
                    self.get_logger().info(f"  相机 {i}: 处理 {self.frame_counts[i]}, 丢弃 {self.drop_counts[i]}, 丢弃率 {rate:.2f}%")
