...
def multi_proc_worker(input_queue, output_queue, im_shape, t, camera_para, max_dis):
    ...
    dra = DRAW(im_shape, t)
    # 在ros2框架下，主进程Node的self.get_logger无法被子进程直接调用。此处的print用于多进程调试，部分终端或运行环境下，子进程的print输出不会显示
    # 这不是因为ros2框架必须用self.get_logger，而是由于python多进程在某些环境/终端下print输出可能会被忽略或不被收集。
    # 为了跨进程输出，可以考虑用日志库logging，并设置合适的handler到文件。如果一定要在ros2主进程Node中输出日志，需要在主进程中收集并打印（通过output_queue传递）。
    while True:
        ...
        try:
            ...
            result = {"cam_idx": cam_idx, "image": im, "timestamp": current_timestamp, "updated_bin": updated_bin}
            output_queue.put(result)
        except Exception as e:
            output_queue.put({"cam_idx": task.get("cam_idx",-1), "error": str(e)})
    end_time = time.time()
    if 'current_timestamp' in locals() and current_timestamp % int(ais_skip_ratio * 1000) < t:
        # 注意: 多进程下，print在有些运行环境不显示。ros2节点日志只能用self.get_logger, 但这里不是Node方法，所以无法直接用。
        print(f"[MultiProcWorker] Camera {cam_idx}  Time cost: {end_time - start_time} seconds")
# ------------------------------------------------------
