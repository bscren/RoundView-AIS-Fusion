...
def data_pre(ais, timestamp):
    ...
    # 否则对其进行推算，并添加到当前时刻的船舶数据中
        ...
        geo_d = pyproj.Geod(ellps="WGS84") # 定义地理坐标系
        
        # 算下一时刻距离
        distance = ais['speed'] * ((timestamp - ais['timestamp']) / 3600) * 1852
        ais['timestamp'] = timestamp

        # 算经纬度,意思即是根据航向和速度，推算出当前时刻的经纬度位置
        ais['lon'], ais['lat'], c = geo_d.fwd(ais['lon'], ais['lat'], ais['course'], distance)
    return ais

def data_pred(AIS_cur, AIS_read, AIS_las, timestamp):
    '''
        分别对当前延迟发送的前几秒的ais数据，对上一时刻未出现的船舶数据进行推算，并添加在当前时刻AIS_current中
        :param AIS_cur: 当前时刻的船舶数据
        :param AIS_read: 当前时刻（in 1s from topic）读取到的船舶数据   
        :param AIS_las: 上一时刻的船舶数据
        :param timestamp:时间戳
    '''
    for index, ais in AIS_read.iterrows():
        ...
        if ais['timestamp'] == int(timestamp//1000):
            # 意思即是当前时刻的AIS数据已经存在，无需推算，直接添加到当前时刻的AIS数据中
...
def data_coarse_process(AIS_current,AIS_last,camera_pos_para,max_dis):
    '''
    数据预处理，包括数据清洗、数据筛选
    ...
    :return: 预处理后的当前时刻船舶数据
    '''
    # camera_pos_para = [msg.longitude, msg.latitude, msg.horizontal_orientation, msg.vertical_orientation, msg.camera_height] + self.camera_para
    camera_loc = (camera_pos_para['latitude'], camera_pos_para['longitude'])
    ...
    for index, ais in AIS_current.iterrows():
        ...
        if dis > max_dis or data_filter(ais, camera_pos_para) == 'ais_del':
            AIS_current = AIS_current.drop(index=index)
    return AIS_current


class AISPRO(object):
    def __init__(self, im_shape, t):
        # 图像尺寸
    ...
    def ais_pro(self, AIS_cur, AIS_las, AIS_vis, camera_pos_para, aisbatch_cache, timestamp):
        ...
        # 2.数据粗清洗 AIS_read -> AIS_read
        AIS_read = data_coarse_process(AIS_read, AIS_las, camera_pos_para, self.max_dis)
                
        # 3.对未出现的AIS数据推算 AIS_read + AIS_las -> AIS_cur
