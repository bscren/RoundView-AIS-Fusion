...
def time2stamp(Time):
    # name format: 2023_04_01_12_30_45_123
    # datetime format: %Y_%m_%d_%H_%M_%S_%f
    # timeStamp 精确到毫秒（微秒四舍五入后）的整数时间戳
...
def read_all(path, result_path):
    # print("read_all path:", path)
    video_path = glob.glob(path+'*.mp4') + glob.glob(path+'*.avi')
    video_path = video_path[0]
    # print("readed video_path" , video_path)
