...
class AisVisNode(Node):
    def __init__(self):
        ...
        self.thread_pool = BoundThreadPoolExecutor(
            max_workers=min(8, self.num_cameras * 2),  # 增加线程数（如最多8个）
    ...
    def synchronized_camera_callback(self, *msgs):
        """处理接收到的多相机同步图像消息"""
        self.get_logger().info("处理接收到的多相机同步图像消息")
        ...
        with self.ais_lock , self.gnss_lock :
        ...
        # 为每个相机启动独立的后台线程处理（传入相机索引、图像、时间戳）
    ...
    def process_single_camera_frame(self, cam_idx, cv_image, current_timestamp, skip_ratio):
        ...
        """处理单个相机的图像帧"""
        # self.get_logger().info(f"\nProcessing camera {cam_idx} frame at timestamp {current_timestamp}")
        ...
        # 线程安全地获取共享数据
            ...
            AIS_vis, AIS_cur = AIS_processor.process(ais_batch, camera_pos, current_timestamp, skip_ratio)

            # 2. 当前相机的视觉轨迹提取（使用专属VIS处理器）
            Vis_tra, Vis_cur = VIS_processor.feedCap(cv_image, current_timestamp, AIS_vis, bin_info, skip_ratio)

            # 3. 当前相机的数据融合（使用专属FUS处理器）
            Fus_tra, updated_bin = FUS_processor.fusion(AIS_vis, AIS_cur, Vis_tra, Vis_cur, current_timestamp, skip_ratio)
            ...
            im = DRA_processor.draw_traj(
                cv_image, AIS_vis, AIS_cur, Vis_tra, Vis_cur, Fus_tra, timestamp=current_timestamp,  skip_ratio = skip_ratio
                # cam_id=cam_idx  # (可选)在绘制时标注相机ID
            )
            ...
            end_time = time.time()
            self.get_logger().info(f"Camera {cam_idx} 处理耗时: {end_time - start_time:.3f}秒")
    ...
    def refresh_window_callback(self):
        ...
        for cam_name, img in current_images.items():
            if img is  None:
                ...
                self.get_logger().info(f"准备显示图像 {cam_name}")
