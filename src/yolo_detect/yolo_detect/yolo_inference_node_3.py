...
class YoloInferenceNode(Node):
    def __init__(self):
        ...
        # 视频配置参数
        self.video_path = "/home/tl/RV/VideonImage/验证集/视频/VID_20230627_140101.mp4"
        # "/home/tl/RV/VideonImage/船舶02.mp4"  # 指定视频地址，需修改为实际路径
        self.SCALE = 0.7  # 图像缩放比例
        self.msg_index_ = 0  # 消息计数器
        self.conf_threshold = 0.5  # 置信度阈值，用于过滤boxes
    ...
    def process_video_frame(self):
        ...
        original_img = results[0].orig_img # raw image
        # 1. 获取并过滤检测框 (filtered_boxes)
        filtered_boxes = self.get_filtered_boxes(results[0])
        ...
        resized_img = cv2.resize(
            original_img,
            (int(original_img.shape[1] * self.SCALE), int(original_img.shape[0] * self.SCALE)),
            interpolation=cv2.INTER_LINEAR
        )
        ...
        jh_msg.message = self.build_json_message(filtered_boxes)
    ...
    def get_filtered_boxes(self, result):
        """从YOLO检测结果中提取并过滤边界框"""
        filtered_boxes = []
        # 遍历所有检测框
        for box in result.boxes:
            # 提取坐标（xyxy格式：左上角x, y，右下角x, y）
            ...
            if conf >= self.conf_threshold:
                filtered_boxes.append({
                    "x1": round(x1*self.SCALE, 2),
                    "y1": round(y1*self.SCALE, 2),
                    "x2": round(x2*self.SCALE, 2),
                    "y2": round(y2*self.SCALE, 2),
    ...
    def build_json_message(self, filtered_boxes):
        """构建与C++版本JHmessagetoJson完全一致的JSON格式"""
        # 如果没有检测到目标，返回空数组
        if not filtered_boxes:
            self.get_logger().info("这一帧build_json_message没找到目标")
            return "[]"
        ...
        for i, box in enumerate(filtered_boxes):
            # 构建单个检测框的JSON对象
            json_str += "{"
            json_str += f"\"class_name\":\"{box['class_name']}\","
            json_str += f"\"confidence\":{box['confidence']},"
            json_str += f"\"x1\":{box['x1']},"
            json_str += f"\"y1\":{box['y1']},"
            json_str += f"\"x2\":{box['x2']},"
            json_str += f"\"y2\":{box['y2']}"
            json_str += "}"
        ...
        json_str += "]"
        self.get_logger().info("完成了一次build_json_message")
        return json_str
