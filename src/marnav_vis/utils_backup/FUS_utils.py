...
def DTW_fast(traj0, traj1):
    ...
    # print(f"FUSION DEBUG: DTW distance: {d}, angle: {theta}")
...
class FUSPRO(object):
    ...
    def cal_similarity(self, AIS_list, AIS_MMSIlist, VIS_list, VIS_IDlist, bin_las):
        # AIS_list, AIS_MMSIlist, VIS_list, VIS_IDlist, bin_las
        # print("FUSION DEBUG: AIS_list:", AIS_list)
        # print("FUSION DEBUG: AIS_MMSIlist:", AIS_MMSIlist)
        # print("FUSION DEBUG: VIS_list:", VIS_list)
        # print("FUSION DEBUG: VIS_IDlist:", VIS_IDlist)
        # print("FUSION DEBUG: bin_las:", bin_las)
        # 1.初始化
        ...
        for i in range(len(VIS_list)):
            for j in range(len(AIS_list)):
                ...
                if int(cur_mmsi) not in bin_MMSI and int(cur_ID) not in bin_ID:
                    ...
                    # 判断是否保存
                    # print(f"FUSION DEBUG: 情况1 Distance for VIS ID {cur_ID} and AIS MMSI {cur_mmsi}: {dis} < self.max_dis {self.max_dis}, Angle: {theta} < math.pi*(5/6)")
                    if dis < self.max_dis and theta < math.pi*(5/6):
                        matrix_S[i][j] = DTW_fast(VIS_list[i], AIS_list[j])
                        # print(f"FUSION DEBUG: 情况1 Calculated DTW distance for VIS ID {cur_ID} and AIS MMSI {cur_mmsi}: {matrix_S[i][j]}")
                    else:
                        matrix_S[i][j] = 1000000000
                        # print(f"FUSION DEBUG: 情况1 No valid match for VIS ID {cur_ID} and AIS MMSI {cur_mmsi}")

                # 情况2: 存在绑定信息时，极小值
                elif cur_IDmmsi in binIDmmsi:
                    matrix_S[i][j] = 0-int(bin_las[bin_las['ID/mmsi'] == cur_IDmmsi]['match'].values)*100
                    # print(f"FUSION DEBUG: 情况2 Found binding for VIS ID {cur_ID} and AIS MMSI {cur_mmsi}, assigned score: {matrix_S[i][j]}")

                # 情况3: 存在绑定信息中但不是绑定值，无穷大
                else:
                    matrix_S[i][j] = 1000000000
                    # print(f"FUSION DEBUG: 情况3 No valid match for VIS ID {cur_ID} and AIS MMSI {cur_mmsi}")
        return matrix_S
    ...
    def traj_match(self, AIS_list, AIS_MMSIlist, VIS_list, VIS_IDlist, AInf_list, VInf_list, timestamp):
        ...
        # 输出检测是否为空
        # if mat_las.empty:
        #     print("FUSION DEBUG: mat_las is empty")
        # elif not mat_las.empty:
        #     print(f"FUSION DEBUG: mat_las length: {len(mat_las)}")
        # if bin_las.empty:
        #     print("FUSION DEBUG: bin_las is empty")
        # elif not bin_las.empty:
        #     print(f"FUSION DEBUG: bin_las length: {len(bin_las)}")
        ...
        # ================ 输出相似性矩阵 ================
        # if matrix_S.size == 0:
        #     print("FUSION DEBUG: similarity matrix_S is empty")
        # elif np.all(matrix_S == 0):
        #     print("FUSION DEBUG: similarity matrix_S contains all zeros")
        # elif np.all(matrix_S == 1000000000):
        #     print("FUSION DEBUG: similarity matrix_S contains all inf values")
        # print("FUSION DEBUG: similarity matrix_S:\n", matrix_S)
        # ================================================
        # 3.匈牙利最优匹配
    ...
    def fusion(self,AIS_vis, AIS_cur, Vis_tra, Vis_cur, timestamp):
        if timestamp % 1000 < self.t:
            ...
            # 输出self.mat_list, self.bin_cur是否为空
            # if self.mat_list.empty:
            #     print("FUSION end DEBUG: self.mat_list is empty")
            # elif not self.mat_list.empty:
            #     print(f"FUSION end DEBUG: self.mat_list length: {len(self.mat_list)}")
            # if self.mat_cur.empty:
            #     print("FUSION end DEBUG: self.mat_cur is empty")
            # elif not self.mat_cur.empty:
            #     print(f"FUSION end DEBUG: self.mat_cur length: {len(self.mat_cur)}")
            # if self.bin_cur.empty:
            #     print("FUSION end DEBUG: self.bin_cur is empty")
            # elif not self.bin_cur.empty:
            #     print(f"FUSION end DEBUG: self.bin_cur length: {len(self.bin_cur)}")
