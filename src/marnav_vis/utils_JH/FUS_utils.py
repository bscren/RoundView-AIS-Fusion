...
def angle(v1, v2):
    """
    :param v1: VIS轨迹的(x,y)列表
    :param v2: AIS轨迹的(x,y)列表
    :return: 轨迹速度夹角(弧度制)
    """
    # 计算轨迹速度的夹角
    ...
    if angle1*angle2 >= 0:
        included_angle = abs(angle1-angle2) # same sign
    else:
        included_angle = abs(angle1) + abs(angle2) # different sign
...
def DTW_fast(traj0, traj1):
    """"
    :param traj0: VIS轨迹的(x,y)列表
    :param traj1: AIS轨迹的(x,y)列表
    :return: DTW距离
    """
    # 1.计算轨迹间夹角
...
def traj_group(df_data, df_dataCur,  kind):
    ...
    :param df_dataCur: 当前时刻的AIS数据或者视频数据
    :param kind: 数据类型：AIS或VIS
    返回  历史中符合当前时刻 分组之后的轨迹列表以及每条轨迹对应的MMSI呼号列表或者ID号列表，为匹配做准备
...
class FUSPRO(object):
    ...
    def initialization(self, AIS_list, VIS_list):
        """
        初始化函数,
        "匹配",意思是关联程度低于self.bin_num次，而“绑定”是关联程度超过self.bin_num次
        :param mat_las: 上一时刻的匹配数据, pd.DataFrame(columns=['ID/mmsi','timestamp', 'match'])
        :param bin_las: 上一时刻的绑定信息, pd.DataFrame(columns=['ID/mmsi', 'timestamp', 'match'])
        :param mat_cur: 当前时刻的匹配数据, pd.DataFrame(columns=['ID/mmsi','timestamp', 'match'])
        :param bin_cur: 当前时刻的绑定信息, pd.DataFrame(columns=['ID', 'mmsi', 'timestamp', 'match'])
        :param mat_list: 当前时刻的匹配信息, pd.DataFrame(columns=['ID', 'mmsi', 'lon', 'lat', 'speed', 'course', 'heading', 'type', 'timestamp'])
    ...
    def cal_similarity(self, AIS_list, AIS_MMSIlist, VIS_list, VIS_IDlist, bin_las):
        """
        计算VIS_list-AIS_list相似性矩阵
        :param AIS_list: 当前时刻AIS轨迹 在历史轨迹中 符合(x,y)列表
        :param AIS_MMSIlist: AIS轨迹对应的MMSI呼号列表
        :param VIS_list: 当前时刻VIS轨迹 在历史轨迹中 符合(x,y)列表
        :param VIS_IDlist: VIS轨迹对应的ID列表
        :param bin_las: 上一时刻的绑定信息
        :return: 相似性矩阵
        """
        ...
        # 按照这个唯一绑定对（ID/mmsi）对 bin_las 进行分组
        if len(bin_las)!=0:
        ...
        for i in range(len(VIS_list)):
            for j in range(len(AIS_list)):
                ...
                if int(cur_mmsi) not in bin_MMSI and int(cur_ID) not in bin_ID:
                    ...
                    # 计算距离
                    x_VIS = VIS_list[i][-1][0] # 取第 i 条视频轨迹, 该轨迹的最后一个[-1] 位置点（即最新的位置）x0 y1
                    y_VIS = VIS_list[i][-1][1] # the same below
    ...
    def data_filter(self, row_ind, col_ind, VIS_list, AIS_list):
        """
        数据滤波函数，删除过远或角度过大数据
        :param row_ind 是视频轨迹（VIS）的索引列表，每个元素对应一条 VIS 轨迹；
        :param col_ind 是 AIS 轨迹的索引列表，每个元素对应一条 AIS 轨迹；
        """
        # 1.初始化
        matches = []
    ...
    def traj_match(self, AIS_list, AIS_MMSIlist, VIS_list, VIS_IDlist, AInf_list, VInf_list, timestamp):
        """
        轨迹匹配主函数
        :param AIS_list: AIS轨迹(x,y)列表
        :param AIS_MMSIlist: AIS轨迹对应的MMSI呼号列表
        :param VIS_list: VIS轨迹(x,y)列表
        :param VIS_IDlist: VIS轨迹对应的ID号列表
        :param AInf_list: AIS轨迹信息列表: mmsi,lon,lat,speed,course,heading,type,x,y,timestamp
        :param VInf_list: VIS轨迹信息列表: ID,x1,y1,x2,y2,x,y,timestamp
